...
public class Shuffler {
	...
	public static void main(String[] args) {
		...
		System.out.println("Results of " + SHUFFLE_COUNT + " selection shuffles:");
	...
	public static boolean arePermutations(int[] check1, int[] check2) {
		for(int checking : check1) {
			for(int check : check2) {
				if(checking == check) return false;
			}
		}
		return true;
	}
	...
	public static String flip() {
		final int tailsChance = (int) Math.random();
		final int headsChance = (int) (Math.random() * 2);
		if(headsChance > tailsChance) return "heads";
		else if(tailsChance > headsChance) return "tails";
		else {
			final int localChance = (int) (Math.random() * 2);
			if(localChance == 1) return "heads";
			else return "tails";
		}
	}
